...
class SelectEquipamentosScreen extends StatefulWidget {
  ...
  const SelectEquipamentosScreen({
    Key? key,
  ...
  @override
  _SelectEquipamentosScreenState createState() =>
      _SelectEquipamentosScreenState();
}

class _SelectEquipamentosScreenState extends State<SelectEquipamentosScreen> {
  final List<String> _equipamentosOptions = [
    'Betoniera',
    'Caminhão Basculante',
    'Compactador de solo',
    'Escavadeira',
  ...
  Future<void> _saveToFirestore() async {
    try {
      // Atualiza o content['Equipamentos'] no relatório
      ...
      await relatorioRef.update({
        'content.Equipamentos': _selectedItems
            .map((title) => {
                  'title': title,
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Equipamentos salvos com sucesso')),
      );
    } catch (error) {
      ScaffoldMessenger.of(context).showSnackBar(
        ...
        body: Padding(
          ...
          child: Column(
            children: [
              ...
              // Lista de checkboxes para equipamentos
              Expanded(
                child: ListView(
                  children: _equipamentosOptions
                      .where((option) => option
                          .toLowerCase()
